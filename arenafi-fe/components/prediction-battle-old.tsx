...
export default function PredictionBattle({ user }: PredictionBattleProps) {
  ...
  const fetchBattles = useCallback(async () => {
    ...
    try {
      ...
      let allBattles;
      try {
        allBattles = await vault.getAllBattles();
        console.log("📊 Raw battles from contract:", allBattles);
        console.log("📊 Number of battles:", allBattles.length);
      } catch (error) {
        console.log("ℹ️ No battles exist yet (empty array), this is normal for a new contract", error);
        setBattles([]);
        return;
      }
  ...
  const handleCreateBattle = async () => {
    ...
    try {
      ...
      // Refresh battles list with a small delay
      console.log("🔄 Refreshing battles list...");
      setTimeout(async () => {
        await fetchBattles();
  ...
  const fetchPriceUpdateData = async (): Promise<string[]> => {
    try {
      ...
      const hexData = data.binary.data || [];
      
      console.log("🔍 Raw hex data from Hermes:", hexData);
      
      // Convert hex strings to proper bytes format by adding '0x' prefix
      const formattedData = hexData.map((hex: string) => {
        const formatted = hex.startsWith('0x') ? hex : `0x${hex}`;
        console.log("🔍 Formatted hex:", formatted.substring(0, 20) + "...");
        return formatted;
      });
  ...
  const handleResolveBattle = async (battleId: bigint) => {
    console.log("🚨 NEW CODE VERSION - UPDATED FUNCTION!");
    ...
    try {
      ...
      console.log("📊 First item type:", typeof priceUpdateData[0]);
      console.log("📊 First item sample:", priceUpdateData[0]?.substring(0, 20) + "...");
      ...
      try {
        ...
        console.error("❌ Failed to get Pyth update fee:", error);
        console.log("🔄 Using fallback fee of 0.0002 ETH");
        updateFee = ethers.parseEther("0.0002");
      }

      console.log("🚀 Sending transaction with fee:", ethers.formatEther(updateFee), "ETH");
      console.log("💰 Current balance:", ethers.formatEther(await provider.getBalance(await signer.getAddress())), "ETH");
